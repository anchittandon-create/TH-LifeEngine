...
import { formatFormForAPI } from "@/lib/lifeengine/promptBuilder";
import { generatePlan, formatErrorMessage } from "@/lib/lifeengine/api";
import { PLAN_TYPE_OPTIONS, DURATION_OPTIONS } from "@/lib/lifeengine/planConfig";
...
export default function CreatePlan() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        profileId: `inline_${Date.now()}`, // Inline profile
        profileSnapshot: {
          id: `inline_${Date.now()}`,
          ...
          availability: {
            days_per_week: 5,
            preferred_slots: [{ start: "09:00", end: "10:00" }]
          }
        },
        plan_type: {
          primary: formData.planTypes[0] || "yoga",
        ...
        duration: {
          unit: formData.duration.includes("week") ? "weeks" : "days",
          value: parseInt(formData.duration.match(/\d+/)?.[0] || "1")
        },
